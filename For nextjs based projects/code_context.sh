#!/bin/bash
# A script to generate a single code context file for a Next.js project.
# It includes a directory tree and the contents of relevant files.
#
# Put this in the root folder of your Next.js project and run:
# ./code_context.sh
...
ignore_dirs=(".git" ".idea" ".vscode" "node_modules" "build" "dist" "coverage" ".nyc_output" "tmp" "temp" "public/static" "storybook-static" ".storybook" "cypress/videos" "cypress/screenshots" ".next" "out" ".cache" ".parcel-cache" ".turbo" ".swc")
...
include_files=("*.js" "*.jsx" "*.ts" "*.tsx" "*.html" "*.css" "*.scss" "*.sass" "*.less" "*.json" "*.md" "*.mdx" "*.yml" "*.yaml" "*.xml" "*.txt" "*.test.js" "*.test.jsx" "*.test.ts" "*.test.tsx" "*.spec.js" "*.spec.jsx" "*.spec.ts" "*.spec.tsx" "package.json" "tsconfig.json" "jsconfig.json" "next.config.js" "next.config.mjs" "next.config.ts" "tailwind.config.js" "tailwind.config.ts" "postcss.config.js" "babel.config.js" ".babelrc" ".eslintrc.js" ".eslintrc.json" ".prettierrc" ".prettierrc.js" ".prettierrc.json" "browserslist" ".editorconfig" ".gitignore" "README.md" "Dockerfile" "*.config.js" "*.config.ts" "*.env" "*.env.local" "*.env.development" "*.env.production" "manifest.json" "robots.txt" "vercel.json" "middleware.ts" "middleware.js")
...
echo "// Next.js Project Code Context" >> "$output_file"
...
echo "✅ Next.js code context has been generated successfully!"
